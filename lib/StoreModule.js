...
class StoreModule {
    constructor(name, fields, storePath) {
        ...
        this.mutationTypesPath = path.normalize(this.storePath + conf.get('mtFileName'));
    ...
    extendIndexFile () {
        const storeModulesIndexPath = this.storePath + 'index.js';
        let indexAst = utils.readAST(storeModulesIndexPath);
        let isModules = false;
        let bodyEntries = [];
        ...
        let code = utils.generate(ast.makeModule(bodyEntries));
        file.writeToFile(storeModulesIndexPath, code);
        console.log(chalk.green('EXTENDED ') + chalk.yellow(storeModulesIndexPath));
    }

    makeMutTypesEntry(name) {
        let mutTypeName = utils.makeMutTypeName(name);
    ...
    extendMutationTypes () {
        const ast = utils.readAST(this.mutationTypesPath);
        const newEntries = this.normalizedFields.map((item) => {
            return this.makeMutTypesEntry(item);
        });
        ast.body.push(...newEntries);
        let code = utils.generate(ast);
        file.writeToFile(this.mutationTypesPath, code);
        console.log(chalk.green('EXTENDED ') + chalk.yellow(this.mutationTypesPath));
    };
}
